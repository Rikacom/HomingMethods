...
//вычисляем угол fi>>
    ...
    if(v_mod1==0) // если цель не летит
        ...
        //M=(0,0)
        //Ta1=(x01-x02,y01-y02)=(Ta1_x,Ta1_y)
        //tan(fi)=Ta1_y/Ta1_x
        //fi=atan(Ta1_y/Ta1_x)
        fi=atan2(*targetData->y_t-y,*targetData->x_t-x);
        }
    else
        {
        ...
        if(gamma==(180*gd_RAD)) //цель будет отдаляться от ракеты
            {
    ...
    //                                  M ракета (x02=x,y02=y,vmod2=v_mod)
            ...
            {
            if(!gamma) //цель будет лететь на встречу ракете
                {

    //          * Ta цель (x01,yo1,vx1,vy1)
    //            ->-
    //                ---  B
    //                     *  O точка встречи
    //               A         ---
    ...
    //                                  M ракета (x02=x,y02=y,vmod2=v_mod)
                ...
                //
                //
                //но пример -> если цель летит слишком бысто, а ракета слишком медленно -> угол бессмысленно выбирать, тк ракета не долетит
